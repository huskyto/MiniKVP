...
MiniKVP takes inspiration from Bitcask, a log-structured key-value store. It intentionally omits most of its powerful features in favor of simplicity.
...
A compact index is kept in memory which includes the keys and an offset value, to quickly locate the entry in the actual store file. The offset points to the start of the entry in the on-disk store.
...
CRCs or other integrity checks will not be included.
